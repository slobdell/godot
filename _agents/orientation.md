...
52. **Don't trust float math across builds, and especially not trig.** Same seed: `+ − × ÷ √` hashed identically native vs wasm, but `sin/cos/atan2/exp` did not (`make det-spike` FLOAT_PROBE). Lockstep code must use `Fixed` (integers).
