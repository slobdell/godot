...
| **Churn: `COMMIT_BONUS` 1.15 → 1.35, ADOPTED** | the lead: *"they seem to just move back and forth indefinitely"*. After the acquire-floor veto failed, the lesson it taught was applied: make STAYING worth more, do not forbid leaving. `make squad-decisions` (laptop, yard, seeds 1/3/7), switches / reversals per unit-min: **1.15 → 18.1 / 0.30; 1.35 → 12.7 / 0.17; 1.60 → 12.1 / 0.27**. 1.60 puts reversals back, so 1.35 is a KNEE, not a direction. Ladder (**laptop**, builder0 was down; both arms same machine): 48 matches yard+pit, gangs v law 5200, control on — 27-21, ELO 1037 v 963, inside noise: does not lose, not proven to win. Adopted on the orchestrator's call because the purpose is legibility and "does not lose" is that bar. Control arm kept as variant `x5p115`. **PRE-REGISTERED (agreed before the run): set it back to 1.15 if, at 96 matches on builder0 (`make remote T="tactics-ladder SIDES=champ=x5p115,commit=x5p ARENAS=yard,pit FACTIONS=gangs,law RUNS=6"`), the 1.35 arm loses by more than the noise band, OR reversals do not hold below 0.20 per unit-minute.** |
...
| **Objectives on N7** (done, `d4f339f7`) | `Objectives` reads `Match.objectives`: `goal(match, team, from)` = nearest not held (else nearest held), `contains`, `held_at`, `all_held`, `all`. Brain contests its own goal; commanders judge from their base (stable across a mirrored pair). The guard is gone: it caught arena's yard/pit (35,336 errors in a match that still produced a winner, lesson 122). `test_objectives` enforces no decider reads `CONTROL_CENTER` / `in_control_zone(` / `control_owner`. DISCOVERY_STATE adds `objectives`; outcome `control_before/after` = objectives held. |
