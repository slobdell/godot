...
| **Churn lever that WORKS on the metric: `commit_bonus`** (`d3abaa04`) | variant `x5c` = champion + `commit_bonus` 1.35. `make squad-decisions` (laptop, yard, seeds 1/3/7) over COMMIT_BONUS: **1.15 (today) 18.1 switches / 0.30 reversals per unit-min; 1.35 → 12.7 / 0.17; 1.60 → 12.1 / 0.27** — 1.35 is the knee. Fewer switches could mean fighting WORSE targets, so it is a variant until a ladder says otherwise. **Ladder pending: builder0 was down (power cut, 2026-09-19).** Run `make remote T="tactics-ladder SIDES=champ=x5p,commit=x5c ARENAS=yard,pit FACTIONS=gangs,law RUNS=3"` when it is back; adopt by setting `COMMIT_BONUS` only if x5c wins |
