...
| **Churn lever that WORKS on the metric: `commit_bonus`** (`d3abaa04`) | variant `x5c` = champion + `commit_bonus` 1.35. `make squad-decisions` (laptop, yard, seeds 1/3/7) over COMMIT_BONUS: **1.15 (today) 18.1 switches / 0.30 reversals per unit-min; 1.35 → 12.7 / 0.17; 1.60 → 12.1 / 0.27** — 1.35 is the knee. Fewer switches could mean fighting WORSE targets, so it is a variant until a ladder says otherwise. **Ladder run on the LAPTOP** (builder0 was down, power cut 2026-09-19), 48 matches, yard+pit, gangs v law 5200, control on: **commit 27-21 (ELO 1037 v 963); yard 15-9, pit 12-12**. 56% over 48 matches is inside noise (one-sided p ~ 0.24): it does NOT lose and is NOT proven to win. Decision is the lead's, because the gain is legibility not win rate. Re-run on builder0 for power: `make remote T="tactics-ladder SIDES=champ=x5p,commit=x5c ARENAS=yard,pit FACTIONS=gangs,law RUNS=6"`; adopt by setting `COMMIT_BONUS` to 1.35 |
