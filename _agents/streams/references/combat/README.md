...
| `faction-matrix-{yard,pit}-hexagon-2026-09-19.json` | **The first faction numbers taken on maps the lead actually plays.** `Arena.ROTATION` became `["yard", "pit"]` at `474abf53` — hexagons at the 140 m bound with off-centre mirrored objective pairs — and every earlier faction number in this project was measured on **foundry**, a square with one central objective that he cut. builder0, n=30 per faction per map, SEEDS=5, the rig temporarily reverted so an overlapping deployment could not contaminate it. **The finding is that the MAPS disagree more than the factions do: the gangs swing 63% → 30% between yard and pit (+33 points, 2.6 SE, the only significant difference in the whole table), while no faction's spread WITHIN a map clears the 25-point bar that n=30 demands.** A picture, not a correction — the lead has deferred balance | `make remote T="faction-matrix ARENA=yard SEEDS=5 TIME=150 JOBS=8"`, then `ARENA=pit` (~10 min each on builder0) |
