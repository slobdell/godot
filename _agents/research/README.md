...
| `brief3.md` | The third brief, 2026-09-20 late: the four things the second reply did not answer, reframed as context-free research problems on one page (corridor authoring cost, attribution under rescaling, perceived variety in finite speech libraries, measuring traversability legibility). Sent by the lead; its reply is filed beside it when it exists |
| `response3_verbatim.md` | The reply to `brief3.md`, ~10,000 words, four sections (corridor authoring, attribution under rescaling, perceived variety in speech libraries, traversability legibility), each with references, methods, failure modes and a falsifiable claim. **Same truncation family as the others** (a dozen mid-word cuts, e.g. "itactive", "Knlse-Blockage", "Groikelihood", and the SMT paragraph of problem 2 is garbled); its "audit" counts and every attribution percentage ("−26.5 % ± 4.2 %") are constructions, not measurements of anything of ours. Curated in `research_catalog.md` *Round 10 addendum, part 2* |
