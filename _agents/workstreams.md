...
| **Unit catalog** (classes, costs, stats, hardpoints, component slots, budget) | `game/units/units.gd`: schema v1 on stream/gameplay (2026-09-15): the simulation reads it; units `tank`, `scout` (and `artillery` when it lands); `COMPONENTS` (`heat_sink`, `ammo_rack`, `shield_booster`, `armor_plating`) with `modifiers`; weapon `cost` in `Weapons.PROFILES`; `Units.cost_of(entry)`, `army_cost(doctrine)`, `validate_loadout(entry)`, `loadout_of(entry)`. v0 keys unchanged | garage (UI), doctrine files |
| **Loadout fields in doctrine JSON** (per tank: `unit`, `weapon` or `weapons` {hardpoint: weapon}, `components` [ids], `paint` "#rrggbb") | read and **validated** by `Doctrine.parse` on stream/gameplay (2026-09-15): unknown units/weapons/components, a weapon a hardpoint doesn't accept, too many components, or bad paint are errors. Armies: ≤ 20 units, ≤ 4 squads, ≤ 5 per squad | garage (produces), gameplay (consumes) |
