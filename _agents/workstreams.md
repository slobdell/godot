...
| **HUD messages**: `Hud.post_message(text: String, severity: int)` with `Hud.INFO` / `WARNING` / `ERROR` | `game/ui/hud.gd` emits `message_posted`, which feeds the `CyberMessages` banners in `hud.tscn` | gameplay posts (orders, losses, results via `MatchAnnouncer`); look & feel renders (banners) |
...
| **Loadout fields in doctrine JSON** (per tank: `unit`, `weapon` or `weapons` {hardpoint: weapon}, `components` [ids], `paint` "#rrggbb" (absent = team color), optional `directive`; the garage mirrors the unit's FIRST hardpoint into `weapon` and adds top-level `garage: {schema, budget, cost}`) | read and **validated** by `Doctrine.parse` (gameplay, 2026-09-15): unknown units/weapons/components, a weapon a hardpoint doesn't accept, too many components, or bad paint are errors. Armies: ≤ 20 units, ≤ 4 squads, ≤ 5 per squad. Written by `game/garage/loadout.gd` (garage) | garage (produces), gameplay (consumes) |

## Invariants every stream must keep

1. `make check` passes before merging (lint, tests, network, combat, match, determinism).
2. **Art never changes the simulation.** `make sim-baseline` (part of `make check`) replays a seeded match and compares its `state_hash` to `tests/baselines/sim_state_hash.txt` (`e5cf33921713b657` on 2026-09-15). Look & feel, assets, garage, and netcode must leave it unchanged. Gameplay changes it on purpose and updates the file in the same commit, saying why.
