...
| `unit.<id>.hull` *(round 2, rules R3; ids: scout, tank, ifv, artillery, lancer, burner)* | like `tank.hull` | the unit's `hull_size` from `Units.PROFILES` (a slot filled by `unit.<id>.hull` is **not** rescaled; the `tank.hull` fallback is scaled from the tank's 2.4 × 1.6 × 3.6) | `set_team_color`, `set_paint`, `set_shield` | ≤ 15k tris |
...
| `unit.artillery.hull` `set_deployed(ratio)` *(round 3, assets X5; combat X5 calls it)* | | 0 = stowed (outrigger legs slid in under the chassis and lifted), 0.5 = beams out, 1 = jacks down (the default, the approved concept's pose) | every frame or on change | rigid parts cut from the generated hull (OutriggerRig), no skeleton |
