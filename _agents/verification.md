...
| 6e | Garage flow | `make garage-smoke` (screenshots: `make garage-shots` → **Read `build/screenshots/garage-*.png`**; browser: `make garage-web-smoke` → `web-garage*.png`) | nothing (shots: a display) | `--garage` opens the garage, FIGHT saves the army to `user://doctrines/` and the skirmish starts with it (`GARAGE_FIGHT … green=N rust=5`), no `ERROR` | anything touching the garage, skirmish startup, doctrines, `GameMode.choose` |
...
**Bundles:** `make check` = rows 0, 1, 2, 6, 6b, 6c, 6d, 6e (all headless, ~2 min). `make check-all` = `check` + rows 3, 4, 5, 7 and fails on any `ERROR` from the exported server shutting down with bots. **Then read the screenshots.**
