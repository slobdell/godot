...
| **Pitch is its own axis** | Zoom sets only the distance. **Page Up / Page Down** or **ctrl+wheel** tilt (22°-50°), **Home** resets it. Round 5 tilted from 25° to 82° as you zoomed out, so seeing your army cost you a top-down view. **O** is the deliberate top-down overview (77°) and back. **The default is the lead's pick** from `make camera-looks`: 25° below the horizon, 50 m out, FOV 60°. |
| **The wall cutaway** | At 25° a squad near a wall (every spawn) is framed from a camera past the wall, inside the grandstand; the camera's near plane then sits just short of where its sight line meets the wall's foot, so the stands between it and the arena aren't drawn (`RtsCamera.cutaway_near`). Over the arena nothing changes. Chosen over raising the pitch near walls, which would bring back the top-down view exactly where every match starts. |
