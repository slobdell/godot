...
| **Pitch is its own axis** | Zoom sets only the distance. **Page Up / Page Down** or **ctrl+wheel** tilt (8°-50°), **Home** resets it. Round 5 tilted from 25° to 82° as you zoomed out, so seeing your army cost you a top-down view — that was the complaint, and decoupling fixed it. **O** is the deliberate top-down overview (77°) and back. **Default 45°** (round 5's start pose). The lead picked 25°, then 12°, from pages of still frames, then played 12° and rejected it: *"I was totally wrong about the camera, the game is unplayable now with low field of view."* **The default pitch is a playability number: choose it from a played session, never from a frame.** |
