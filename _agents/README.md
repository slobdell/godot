...
- **[tactical_map.md](tactical_map.md)**: **How the player commands.** The few-inputs UX (who / where / how), commander election and succession, formations and drills (after Army tank platoon doctrine), the SquadCommand data flow, skirmish mode, and what's verified. Read before touching squads, the map UI, or player-facing controls.
- **[tank_brain.md](tank_brain.md)**: **The deterministic CPU middle layer.** Autonomous tank brains (utility AI over directive weights), directives and doctrines as structured data, shared team vision, weapons as data (flamethrower), the determinism contract, the squad-command UI options, and experiments T0–T3 with results. Read before touching AI, doctrines, or weapons.
- **[agent_bridge.md](agent_bridge.md)**: How Claude plays a tank over a localhost HTTP bridge as a *commander* (standing orders like `move_to` and `target`, not per-frame inputs). It serves as a smoke test, a prototype of the LLM commander, and the first version of the AI action layer. Read before touching orders, observations, or `tools/agent.py`.
