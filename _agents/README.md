...
- **[architecture.md](architecture.md)**: The layers (simulation → controllers → skills → doctrine → authoring) and the one seam everything hangs on, `TankCommand`. Also: why the LLM never runs inside the game loop. Read before adding a new kind of "thing that decides what a tank does".
