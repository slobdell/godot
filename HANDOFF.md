...
_Last updated: 2026-09-20 18:00. **Round 9's overnight run: sixteen branches and all three checkpoints merged; the day has merged twenty-four more. MAIN CHECK on `0ad28f49` (builder0, 1192 s, 5 shards): 1516 passed, 2 failed, 0 engine errors, `sim-baseline 1e90f69e5d6fcc46 (baseline unmoved)`, `determinism 253adefeec657df1`; the two reds are the spawn test (armed diagnosis, squad's; the lateral push is being measured by scale) and `test_combat_sim_cost` under the drain guard's 30-frame budget (raised to 120 at `ccdf144d`). **`main-checked` = the last main tip whose own check RAN (`0ad28f49`); it is not green: its two reds are listed above and merging it inherits them.** Since then main has taken control's item 4, combat's hull geometry (disc default), nav's verified drain set and budget, scale's revert and lamps; a main check on the tip follows. The last fully green `main` is `0808834e`. Read the morning summary first; the red-test paragraph under "Where it stood" is the full history.**_
