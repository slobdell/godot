...
- **feel:** DONE: the lamp pair is a real pair now (both halves show-off, the before from the pre-lamp arena: the six lamps light the floor), and the Terminus roof dressing (`19e258d2`, checking: rooftop plant at zero extra draw calls, inside the collision box; the first version passed every test and read as nothing, caught only by looking; the three-way sent to you). Owed: the hinge cost with `TUNE=match.no_damage=1` (the tune is on main) if the box allows; the per-faction hull rim light is round 10 pending your call.
