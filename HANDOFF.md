...
_Last updated: 2026-09-20 14:00. **Round 9's overnight run: sixteen branches and all three checkpoints merged; the morning has merged eleven more. `main` at `167e3541` is RED on ONE test and the scenario gate: the foundry spawn test (NOT a transient after all: feel's shard repro shows the widest hulls abreast converge to a 3.15 m lateral pitch where 3.33 m is needed, a wedge-pitch defect against the widened roster; squad fixes the pitch at the source, keeps the assertion) and three scenario rows (dodge: known-failing, metrics' PENDING marker landing; engine deck: the scout never gets past the tank's side since the resize, scale's discriminator queued; suppression: the margin did not recover with the artillery box, combat reads the bar). The city-block and artillery reds are CLEARED (feel). Baseline `1e90f69e5d6fcc46`, recorded twice. The last fully green `main` is `0808834e`. Read the morning summary first; the red-test paragraph under "Where it stood" is the full history with six retracted diagnoses.**_
