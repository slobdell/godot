...
- **combat:** 64367efe's check (15:00): 1476 passed, 18 failed, 14 of them one cascade — `Navigation map synchronization had 284 edge error(s)` poisoning shard 0 (the runner fails any test that logs an engine error), none combat's; routed to nav with the shard order; sim-baseline unreached twice in a row because `check` aborts on `test` (metrics makes the hash targets run regardless). Landing now: the suppression separation assertion and `Units.hull_distance_to_line` / `hull_reach_along` behind `match.hull_disc`. Then **the hull-rotation plant defect**: a hull's position is collision-resolved and its rotation is not, so
