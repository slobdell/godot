...
_Last updated: 2026-09-20 14:00. **Round 9's overnight run: sixteen branches and all three checkpoints merged; the morning has merged eleven more. `main` at `167e3541` is RED on ONE test and the scenario gate: the foundry spawn test (NOT a transient after all: feel's shard repro shows the widest hulls abreast converge to a 3.15 m lateral pitch where 3.33 m is needed, placement clears by 1.67 m, so the depenetration step that sank three units also moves the widest pairs ~0.9 m toward each other, laterally; squad's failure message names each pair's four numbers and displacement vectors; scale measures the vectors across the first two steps) and three scenario rows (dodge: known-failing, metrics' PENDING marker landing; engine deck: the scout never gets past the tank's side since the resize, scale's discriminator queued; suppression: the margin did not recover with the artillery box, combat reads the bar). The city-block and artillery reds are CLEARED (feel). Baseline `1e90f69e5d6fcc46`, recorded twice. The last fully green `main` is `0808834e`. Read the morning summary first; the red-test paragraph under "Where it stood" is the full history with six retracted diagnoses.**_
...
- **combat:** 64367efe's check (15:00): 1476 passed, 18 failed, 14 of them one cascade — `Navigation map synchronization had 284 edge error(s)` poisoning shard 0 (the runner fails any test that logs an engine error), none combat's; nav reproduced the same error tonight (two arenas alive in one process, `ai_scenario.gd:155` warns of it) and takes the fix holding `game/arena/` while arena rests, with combat sending the one fact (the test before the first carrier, and whether it builds an arena); sim-baseline unreached twice in a row because `check` aborts on `test` (metrics makes the hash targets run regardless). Landing now: the suppression separation assertion and `Units.hull_distance_to_line` / `hull_reach_along` behind `match.hull_disc`. Then **the hull-rotation plant defect**: a hull's position is collision-resolved and its rotation is not, so
