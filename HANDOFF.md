...
arrived with CP2, not with any nav change. **A third consequence, on the unit you named (scale, 12:10):** `roster-scale` prints `no mesh` for two units, the Condemned `tank` (your bus-tank, `Units.DEFAULT`) and `burner`; the rule gave them a length from the reference and kept today's width and height (the brief's "do not invent proportions"), so the bus-tank is 8.62 m long and still 2.40 m wide and 2.40 m tall, a 3.6:1 slab where a Type D school bus is ~2.6 × 3.1 m and 4.7:1. Invisible to every check (no mesh to compare the box against). **Decided on your behalf (12:15):** width and height for those two come from the cited reference's published dimensions × K, the same rule the length follows, with the source beside each; scale lands it as its own baseline move after the artillery's and re-shoots the lineup so you can overrule from the picture. **Consequence (13:40, scale `adcdec13`): the dozer narrowed 2.40 → 1.83 m and `test_combat_screening` reports the Lancer behind it loses 39.7 % of what it loses alone, against a 0.35 bar; the screen still works (every other assertion holds), it works less well by a measured amount. Ruled: the bar was calibrated against an inherited width and combat re-derives it with the width beside it; if you prefer the dozer wide for its role, that is the override the frame is for.** The round's artefact from this: scale writes the list of every consumer of `hull_size` and what each assumes (three surprises this round: the spawn grid, the asset contract's refit, the screening bar).
