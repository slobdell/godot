...
_Last updated: 2026-09-20 19:45. **Round 9's overnight run: sixteen branches and all three checkpoints merged; the day has merged twenty-nine more. MAIN CHECK on `0416274d` (builder0, 4 shards): 1506 passed, 23 FAILED, `sim-baseline 1e90f69e5d6fcc46 (baseline unmoved)`, `determinism 253adefeec657df1`; 22 of the 23 are one shard poisoned by the navmesh cascade AGAIN: `test_combat_sim_cost` really holds an arena past teardown (2 regions still there after the drain's 120-frame budget, 1 body), the next tests baked over them, and the runner now fails a test on the engine's 284-edge-error WARNING; combat reproduced it alone on its tree (the earlier "does not reproduce alone" was the shard order): the test calls `teardown()` explicitly inside its loop WITHOUT `await`, so a detached coroutine sits in the 120-frame drain while the loop builds the next arena (a defect in itself), and the residue is exactly one arena's two regions (`$Navigation` plus `Arena._bake()`'s `NavigationMirror`, which shares the same `NavigationMesh` SubResource); **Then the print (combat, 19:55): NO holder. With the teardown awaited, every boundary reads arena freed, match freed, regions 0, bodies 0; the bare `teardown()` detached a drain that sat in its 120-frame wait while the loop built the NEXT unit's arena and counted that live arena as leftover, which is why the message arrived twice; the shared mesh is exonerated and `arena.tscn` untouched. The test now asserts the fixture gone, regions 0 and bodies 0 at each boundary. Still open: the main check's shard-2 cascade (284 edge errors into 22 tests) has no named first carrier yet; the next main check, with metrics' one-cause-many-victims grouping, names it.** nav adds containment (teardown frees leftovers itself after the budget) and a guard against a second concurrent drain. **And the shape is in five more files (combat's grep): `test_control_panel.gd:143` calls `teardown()` mid-test un-awaited, and `test_command_readability.gd:141`, `test_touch.gd:177`, `test_command_camera.gd:194`, `test_tactics_deform.gd:42` call `super.teardown()` un-awaited inside `func teardown() -> void` overrides, so the runner's await returns at once and those viewport-resizing tests have silently skipped the drain since it existed. A teardown that must be awaited but can be declared `-> void` cannot be made safe by review: nav seals the drain into a `_teardown()` the runner awaits, the overridable hook synchronous; control's four and squad's one then stop calling super.** The guard's budget and severity stay: it reported what it saw, and a test lied to it. The 23rd is the spawn test, whose measured fix (a settle tick) is combat's next commit. **`main-checked` stays at `0ad28f49` (annotated: 1516/2, reds listed); it is not green.** The last fully green `main` is `0808834e`. Read the morning summary first; the red-test paragraph under "Where it stood" is the full history.**_
