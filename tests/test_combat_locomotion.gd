...
func test_wheels_do_not_rotate_standing_still() -> void:
	var poses := TankMotion.predict(_car(), 0.0, 0.0, 60)
...
func test_wheel_yaw_rate_is_speed_over_radius() -> void:
	...
	var expected := rad_to_deg(6.0 / 6.0) / 60.0
	...
	assert_near(_yaw_deg(half[0]["forward"]), rad_to_deg(6.0 / 12.0) / 60.0, 0.005, "half lock = twice the radius")
	...
	assert_near(_yaw_deg(fast[0]["forward"]), 100.0 / 60.0, 0.02, "at speed the yaw rate caps at hull_turn_rate_deg")
...
func test_momentum_coasts_and_brakes() -> void:
	var car := _car(14.0)
	var coast := TankMotion.predict(car, 0.0, 0.0, 60)
	var stop_ticks := 0
	for pose: Dictionary in coast:
		if float(pose["speed"]) > 0.0:
			stop_ticks += 1
	assert_near(float(stop_ticks), 14.0 / 20.0 * 60.0, 2.0, "off the throttle it sheds braking_mps2")
	var reverse := TankMotion.predict(_car(14.0), -1.0, 0.0, 60)
