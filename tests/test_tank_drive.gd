...
func test_full_throttle_drives_forward() -> void:
	...
	tank.command = TankCommand.new(1.0, 0.0, Vector3(0, 0, -50))
	await wait_physics_frames(60)
	assert_true(tank.global_position.z < -4.0,
			"1 s at full throttle moves >4 m along -Z (got z=%.2f)" % tank.global_position.z)
	assert_near(tank.global_position.x, 0.0, 0.05, "driving straight does not drift sideways")
...
func test_turn_input_rotates_hull() -> void:
	var tank := _spawn_tank()
	tank.command = TankCommand.new(0.0, 1.0)
...
func test_turret_tracks_aim_point() -> void:
	var tank := _spawn_tank()
	tank.command = TankCommand.new(0.0, 0.0, Vector3(20, 0, 0))
