...
func brain_tank(team: int, tank_name: String, position: Vector3, yaw: float, directive := {},
	...
	var loadout := {"unit": unit, "weapons": {} if weapon == "" else {"main": weapon}}
	var tank := game_match.add_brain_tank(team, squad, weapon if weapon != "" else Weapons.DEFAULT, [directive],
			tank_name, loadout)
...
func shooter(team: int, tank_name: String, position: Vector3, yaw: float, move := {"type": "stop"},
	...
	var loadout := {"unit": unit, "weapons": {} if weapon == "" else {"main": weapon}}
	var tank := game_match.spawn_tank(tank_name, 0, team, weapon if weapon != "" else Weapons.DEFAULT, loadout)
	_place(tank, position, yaw)
