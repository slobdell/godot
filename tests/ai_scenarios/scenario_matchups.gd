...
const PENDING := ["test_a_scout_circles_a_tank_instead_of_trading_frontally", "test_an_ifv_prioritizes_scouts"]
...
func test_a_scout_circles_a_tank_instead_of_trading_frontally() -> void:
	...
	print("MEASURE ai_scout_orbit %.0f deg/s around the tank over %.1f s close (turret %.0f deg/s), scout alive %s" % [
			degrees_per_second, close_ticks / 60.0, turret, scout.is_alive()])
