...
func _run_probe() -> void:
	...
	if not ARMS.has(arm):
			print("DEFILE_NAV asks_refused=%d yields_started=%d guard_rescues=%d orca_solved=%d orca_deflected=%d" % [
			Movement.asks_refused, Movement.yields_started, Movement.guard_rescues, Avoidance.solved, Avoidance.deflected])
